        ...
        if (user.username) {
            var obj = {
                ...
                paper: {
                    ...
                    par: Number($("input[name='par']").val()),
                    qty: Number($("select[name='qty']").val()),
                    discount: Number($("select[name='discount']").val()),
                    maturity: Number($("input[name='maturity']").val()),

                    ownername: Number($("select[name='owner']").val()),
                    owner: [],
                    issuer: user.name,
        ...
        for (var i in entries) {
            ...
            if (entries[i].quantity > 0) {													//cannot buy when there are none
                ...
                if (excluded(entries[i], filter)) {
                    var style;
                    if (user.name.toLowerCase() === entries[i].issuer.toLowerCase()) {
                        //cannot buy my own stuff
                        style = 'invalid';
                        entries[i].owner="hardcode";
                        flag = 1;
                    }
...
function excluded(entry, filter) {
    ...
    if (filter.cusip && filter.cusip !== "" && entry.cusip.toUpperCase().indexOf(filter.cusip.toUpperCase()) == -1) return false;
